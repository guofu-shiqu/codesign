...
const projectDir = resolve(process.env.CODESIGN_PROJECT_DIR ?? process.cwd())
const canvasDir = resolve(process.env.CODESIGN_CANVAS_DIR ?? join(projectDir, 'canvas'))
const canvasFile = join(canvasDir, 'codesign-canvas.json')
const selectionFile = join(canvasDir, 'codesign-selection.json')
const viewStateFile = join(canvasDir, 'codesign-view-state.json')
...
const canvasFileName = 'codesign-canvas.json'
