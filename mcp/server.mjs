...
const CANVAS_FILE_NAME = "codesign-canvas.json";
...
function resolveCanvasDir(args = {}) {
  ...
  const envCanvasDir = nonEmptyString(process.env.CODESIGN_CANVAS_DIR);
  if (envCanvasDir) return pathResolve(envCanvasDir);

  const envProjectDir = nonEmptyString(process.env.CODESIGN_PROJECT_DIR);
...
function resolveSelectionFile(args = {}) {
  return join(resolveCanvasDir(args), "codesign-selection.json");
}

function resolveViewStateFile(args = {}) {
  return join(resolveCanvasDir(args), "codesign-view-state.json");
}
...
function normalizeCoDesignUrl(args = {}) {
  const value = nonEmptyString(args.codesignUrl) || nonEmptyString(process.env.CODESIGN_URL) || "http://127.0.0.1:43217";
...
async function loadCanvasSnapshot(args) {
  const codesignUrl = normalizeCoDesignUrl(args);
...
async function saveCanvasSnapshot(codesignUrl, snapshot) {
  return fetchJson(`${codesignUrl}/api/canvas`, {
    method: "PUT",
...
async function insertCoDesignImage(args = {}) {
  ...
  const { codesignUrl, snapshot } = await loadCanvasSnapshot(args);
  ...
  if (anchorShapeId && !shapeMeta.codesignAnnotationSourceShapeId) {
    shapeMeta.codesignAnnotationSourceShapeId = anchorShapeId;
  }
  if (nonEmptyString(args.annotationScreenshot) && !shapeMeta.codesignAnnotationScreenshot) {
    shapeMeta.codesignAnnotationScreenshot = nonEmptyString(args.annotationScreenshot);
  }
  ...
  if (!args.dryRun) {
    ...
    await saveCanvasSnapshot(codesignUrl, snapshot);
  }
...
async function handleToolCall(id, params) {
  ...
  if (params?.name === TOOL_INSERT_IMAGE) {
    const result = await insertCoDesignImage(params.arguments ?? {});
    sendResult(id, {
      content: [
        {
          type: "text",
